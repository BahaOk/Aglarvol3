...
def home():
    ...
    try:
        ...
        flash("Bir hata oluştu. Lütfen daha sonra tekrar deneyin.", "error")
        return redirect(url_for("kayitOl"))
def chatroom_set_or_get(user2,roomid):
    room_id = roomid
    room_data = {
...
def add_messages_data(sender_id,reciver_id,roomid,content,time,durum):
    room_id = roomid
    message_data = {
        "sender" : session['kullaniciAdi'],
        "sender_id" : sender_id,
...
def private_chat(target_user_id, target_username):
    ...
    chatroom_set_or_get(target_user_id,room_id)
    ...
    messages_ref = db.collection("Chatrooms").document(room_id).collection("Messages").get()
    mes = []
    for m in messages_ref :
        mesaj_bilgisi = m.to_dict()
        mes.append(mesaj_bilgisi)
    
    return render_template("private_chat.html", room_id=room_id, target_user=target_username, m = mes)
...
def handle_message(data):
    ...
    add_messages_data(sender_user_id,target_user_id,room_id,data['message'],message_data["timestamp"],message_data["status"])
...
def on_join():
    ...
    for message in db.collection('Chatrooms').document(room_id).collection("Messages"):
        if message["sender"] == user_id and message["status"] != "Görüldü":
